  ...
  async function showtime(event) {
      ...
      await fetchData(loc).then( async(res) => {
        ...
        await fetchData(url).then( async(data) => {
                ...
                <div class="p-10">Today :-</div>
                <div class="p-10">${data?.results?.sunrise}</div>
                <div class="p-10">${data?.results?.sunset}</div>
                <div class="p-10">${data?.results?.dawn}</div>
                <div class="p-10">${data?.results?.dusk}</div>
                <div class="p-10">${data?.results?.day_length}</div>
                <div class="p-10">${data?.results?.solar_noon}</div>
                <div class="p-10">${data?.results?.timezone}</div>              
              `
        }).catch((err) => console.log(`${err}`))
  
        await fetchData(tomorrowUrl).then( async(data) => {
              document.querySelector('#tomorrow').innerHTML = `
                <div class="p-10">Tomorrow :-</div>
                <div class="p-10">${data?.results?.sunrise}</div>
                <div class="p-10">${data?.results?.sunset}</div>
                <div class="p-10">${data?.results?.dawn}</div>
                <div class="p-10">${data?.results?.dusk}</div>
